...
import (
	...
	"github.com/NexusGPU/gpu-go/second-party/hypervisor/api"
	"github.com/NexusGPU/gpu-go/second-party/hypervisor/framework"
...
func (b *SingleNodeBackend) discoverWorkers() {
	workers, err := b.fileState.LoadWorkers()
	if err != nil {
		log.Printf("Error: Failed to load workers from file state: %v", err)
		return
	}
...
func (b *SingleNodeBackend) notifySubscribers(worker *api.WorkerInfo) {
	...
	for _, ch := range b.subscribers {
		...
		case ch <- worker:
		default:
			log.Printf("Warning: Channel is full, skipping notification for worker change %s", worker.WorkerUID)
		}
	}
}
...
func (b *SingleNodeBackend) StartWorker(worker *api.WorkerInfo) error {
	...
	b.notifySubscribers(worker)
	log.Printf("Info: Worker started: %s", worker.WorkerUID)
	return nil
}
...
func (b *SingleNodeBackend) StopWorker(workerUID string) error {
	...
	log.Printf("Info: Worker stopped: %s", workerUID)
	return nil
}
	...
	return framework.DeviceChangeHandler{
		OnAdd: func(device *api.DeviceInfo) {
			if err := b.fileState.AddDevice(device); err != nil {
				log.Printf("Error: Failed to save device to file state: %v", err)
			} else {
				log.Printf("Info: Device added: %s", device.UUID)
			}
		},
		OnRemove: func(device *api.DeviceInfo) {
			if err := b.fileState.RemoveDevice(device.UUID); err != nil {
				log.Printf("Error: Failed to remove device from file state: %v", err)
			} else {
				log.Printf("Info: Device removed: %s", device.UUID)
			}
		},
		OnUpdate: func(oldDevice, newDevice *api.DeviceInfo) {
			if err := b.fileState.UpdateDevice(newDevice); err != nil {
				log.Printf("Error: Failed to update device in file state: %v", err)
			} else {
				log.Printf("Info: Device updated: %s", newDevice.UUID)
			}
		},
	}
}
